 ...
 * Page class
 */
class DokterList extends Dokter
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'dokter';

    // Page object name
    public $PageObjName = "DokterList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fdokterlist";
    ...
    public function __construct()
    {
        ...
        // Table object (dokter)
        if (!isset($GLOBALS["dokter"]) || get_class($GLOBALS["dokter"]) == PROJECT_NAMESPACE . "dokter") {
            $GLOBALS["dokter"] = &$this;
        }
        ...
        $this->AddUrl = "dokteradd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "dokterdelete";
        $this->MultiUpdateUrl = "dokterupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'dokter');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fdokterlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("dokter"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fdokterlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field nama_dokter
        $this->nama_dokter->AdvancedSearch->SearchValue = @$filter["x_nama_dokter"];
        $this->nama_dokter->AdvancedSearch->SearchOperator = @$filter["z_nama_dokter"];
        $this->nama_dokter->AdvancedSearch->SearchCondition = @$filter["v_nama_dokter"];
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canAdd()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"dokter\" data-caption=\"" . $copycaption . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'AddBtn',url:'" . HtmlEncode(GetUrl($this->CopyUrl)) . "'});\">" . $Language->phrase("CopyLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'praktik_poli')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("praktikpolilist?" . Config("TABLE_SHOW_MASTER") . "=dokter&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("PraktikPoliGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'dokter')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'dokter')) {
            ...
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'dokter')) {
                $caption = $Language->phrase("MasterDetailCopyLink");
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=praktik_poli");
    ...
    protected function setupOtherOptions()
    {
        ...
        if (IsMobile()) {
            ...
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"dokter\" data-caption=\"" . $addcaption . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'AddBtn',url:'" . HtmlEncode(GetUrl($this->AddUrl)) . "'});\">" . $Language->phrase("AddLink") . "</a>";
        }
        ...
        $detailTableLink = "";
                ...
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'dokter') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdokterlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdokterlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fdokterlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        $links = "";
        $btngrps = "";
        $sqlwrk = "`dokter_id`=" . AdjustSql($this->id->CurrentValue, $this->Dbid) . "";
        ...
        if ($this->DetailPages && $this->DetailPages["praktik_poli"] && $this->DetailPages["praktik_poli"]->Visible && $Security->allowList(CurrentProjectID() . 'praktik_poli')) {
            ...
            $url = "praktikpolipreview?t=dokter&f=" . Encrypt($sqlwrk);
            ...
            if ($Security->allowList(CurrentProjectID() . 'dokter')) {
                ...
                $links .= $link;
                $detaillnk = JsEncodeAttribute("praktikpolilist?" . Config("TABLE_SHOW_MASTER") . "=dokter&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
    ...
    protected function loadSearchValues()
    {
        ...
        // nama_dokter
        if (!$this->isAddOrEdit() && $this->nama_dokter->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->nama_dokter->AdvancedSearch->SearchValue != "" || $this->nama_dokter->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$this->nama_dokter->Raw) {
                $this->nama_dokter->AdvancedSearch->SearchValue = HtmlDecode($this->nama_dokter->AdvancedSearch->SearchValue);
            }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fdokterlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"doktersearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"dokter\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'doktersearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;

        // Search highlight button
        $item = &$this->SearchOptions->add("searchhighlight");
        $item->Body = "<a class=\"btn btn-default ew-highlight active\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("Highlight") . "\" data-caption=\"" . $Language->phrase("Highlight") . "\" data-toggle=\"button\" data-form=\"fdokterlistsrch\" data-name=\"" . $this->highlightName() . "\">" . $Language->phrase("HighlightBtn") . "</a>";
