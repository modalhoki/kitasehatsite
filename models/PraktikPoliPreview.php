 ...
 * Page class
 */
class PraktikPoliPreview extends PraktikPoli
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PraktikPoliPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "fasilitas_rumah_sakit") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=fasilitas_rumah_sakit&" . GetForeignKeyUrl("fk_id", $this->fasilitas_rumah_sakit_id->QueryStringValue) . "";
        }
        if ($masterTblVar == "dokter") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=dokter&" . GetForeignKeyUrl("fk_id", $this->dokter_id->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeysFromFilter($f)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "fasilitas_rumah_sakit") {
            $find = "`fasilitas_rumah_sakit_id`=";
            $x = strpos($f, $find);
            if ($x !== false) {
                $x += strlen($find);
                $val = substr($f, $x);
                $val = $this->unquoteValue($val, "DB");
                 $this->fasilitas_rumah_sakit_id->setQueryStringValue($val);
            }
        }
        if ($masterTblVar == "dokter") {
            $find = "`dokter_id`=";
            ...
            if ($x !== false) {
                ...
                $val = substr($f, $x);
                $val = $this->unquoteValue($val, "DB");
                 $this->dokter_id->setQueryStringValue($val);
            }
        }
    }
