 ...
 * Page class
 */
class RumahSakitList extends RumahSakit
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RumahSakitList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "frumah_sakitlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "rumahsakitadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "rumahsakitdelete";
        $this->MultiUpdateUrl = "rumahsakitupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option frumah_sakitlistsrch";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "frumah_sakitlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field daerah_id
        $this->daerah_id->AdvancedSearch->SearchValue = @$filter["x_daerah_id"];
        $this->daerah_id->AdvancedSearch->SearchOperator = @$filter["z_daerah_id"];
        $this->daerah_id->AdvancedSearch->SearchCondition = @$filter["v_daerah_id"];
        $this->daerah_id->AdvancedSearch->SearchValue2 = @$filter["y_daerah_id"];
        $this->daerah_id->AdvancedSearch->SearchOperator2 = @$filter["w_daerah_id"];
        $this->daerah_id->AdvancedSearch->save();

        // Field foto_rumah_sakit
        $this->foto_rumah_sakit->AdvancedSearch->SearchValue = @$filter["x_foto_rumah_sakit"];
        $this->foto_rumah_sakit->AdvancedSearch->SearchOperator = @$filter["z_foto_rumah_sakit"];
        $this->foto_rumah_sakit->AdvancedSearch->SearchCondition = @$filter["v_foto_rumah_sakit"];
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->daerah_id->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_fasilitas_rumah_sakit"
        $item = &$this->ListOptions->add("detail_fasilitas_rumah_sakit");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'fasilitas_rumah_sakit') && !$this->ShowMultipleDetails;
        $item->OnLeft = false;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("fasilitas_rumah_sakit");
    ...
    public function renderListOptions()
    {
        ...
        // "detail_fasilitas_rumah_sakit"
        $opt = $this->ListOptions["detail_fasilitas_rumah_sakit"];
        if ($Security->allowList(CurrentProjectID() . 'fasilitas_rumah_sakit')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("fasilitas_rumah_sakit", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("fasilitasrumahsakitlist?" . Config("TABLE_SHOW_MASTER") . "=rumah_sakit&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("FasilitasRumahSakitGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'rumah_sakit')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=fasilitas_rumah_sakit");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "fasilitas_rumah_sakit";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'rumah_sakit')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fasilitas_rumah_sakit");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "fasilitas_rumah_sakit";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'rumah_sakit')) {
                $caption = $Language->phrase("MasterDetailCopyLink");
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=fasilitas_rumah_sakit");
                ...
                $detailCopyTblVar .= "fasilitas_rumah_sakit";
            }
            if ($links != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_fasilitas_rumah_sakit");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=fasilitas_rumah_sakit");
                $detailPage = Container("FasilitasRumahSakitGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'rumah_sakit') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "fasilitas_rumah_sakit";
                }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"frumah_sakitlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"frumah_sakitlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.frumah_sakitlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // daerah_id
        if (!$this->isAddOrEdit() && $this->daerah_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->daerah_id->AdvancedSearch->SearchValue != "" || $this->daerah_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // foto_rumah_sakit
        if (!$this->isAddOrEdit() && $this->foto_rumah_sakit->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->foto_rumah_sakit->AdvancedSearch->SearchValue != "" || $this->foto_rumah_sakit->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"frumah_sakitlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"rumahsakitsearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"rumah_sakit\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'rumahsakitsearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;

        // Search highlight button
        $item = &$this->SearchOptions->add("searchhighlight");
        $item->Body = "<a class=\"btn btn-default ew-highlight active\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("Highlight") . "\" data-caption=\"" . $Language->phrase("Highlight") . "\" data-toggle=\"button\" data-form=\"frumah_sakitlistsrch\" data-name=\"" . $this->highlightName() . "\">" . $Language->phrase("HighlightBtn") . "</a>";
    ...
    public function listOptionsRendered()
    {
        ...
        //$this->ListOptions["view"]->Body = "";
        //$this->ListOptions["edit"]->Body = "<a class=\"ew-row-link ew-edit\" title=\"\" data-caption=\"Edit\" href=\"/satusehat/webusersedit/".$this->id->CurrentValue."?showdetail=\" data-original-title=\"Edit\"><i data-phrase=\"EditLink\" class=\"icon-edit ew-icon\" data-caption=\"Edit\"></i></a>";
        //$this->ListOptions["copy"]->Body = "";
        //$this->ListOptions["delete"]->Body = "";
    }
