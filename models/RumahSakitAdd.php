 ...
 * Page class
 */
class RumahSakitAdd extends RumahSakit
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RumahSakitAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("rumahsakitlist"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "rumahsakitlist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "rumahsakitview") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->daerah_id->CurrentValue = null;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'daerah_id' first before field var 'x_daerah_id'
        $val = $CurrentForm->hasValue("daerah_id") ? $CurrentForm->getValue("daerah_id") : $CurrentForm->getValue("x_daerah_id");
        if (!$this->daerah_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->daerah_id->Visible = false; // Disable update for API request
            } else {
                $this->daerah_id->setFormValue($val);
            }
        }

        // Check field name 'foto_rumah_sakit' first before field var 'x_foto_rumah_sakit'
        $val = $CurrentForm->hasValue("foto_rumah_sakit") ? $CurrentForm->getValue("foto_rumah_sakit") : $CurrentForm->getValue("x_foto_rumah_sakit");
        if (!$this->foto_rumah_sakit->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->foto_rumah_sakit->Visible = false; // Disable update for API request
            } else {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->daerah_id->ViewValue = $this->daerah_id->Lookup !== null && is_array($this->daerah_id->Lookup->Options) ? $curVal : null;
            }
            if ($this->daerah_id->ViewValue !== null) { // Load from cache
                $this->daerah_id->EditValue = array_values($this->daerah_id->Lookup->Options);
                if ($this->daerah_id->ViewValue == "") {
                    $this->daerah_id->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->daerah_id->CurrentValue, DATATYPE_NUMBER, "");
                }
    ...
    protected function validateForm()
    {
        ...
        if ($this->daerah_id->Required) {
            if (!$this->daerah_id->IsDetailKey && EmptyValue($this->daerah_id->FormValue)) {
                $this->daerah_id->addErrorMessage(str_replace("%s", $this->daerah_id->caption(), $this->daerah_id->RequiredErrorMessage));
            }
        }
        ...
        if (in_array("fasilitas_rumah_sakit", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // daerah_id
        $this->daerah_id->setDbValueDef($rsnew, $this->daerah_id->CurrentValue, 0, false);
        ...
        if ($addRow) {
            ...
            $detailPage = Container("FasilitasRumahSakitGrid");
            if (in_array("fasilitas_rumah_sakit", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->rumah_sakit_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "fasilitas_rumah_sakit"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->rumah_sakit_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
