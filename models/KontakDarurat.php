...
/**
 * Table class for kontak_darurat
 */
class KontakDarurat extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'kontak_darurat';
        $this->TableName = 'kontak_darurat';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`kontak_darurat`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id
        $this->id = new DbField('kontak_darurat', 'kontak_darurat', 'x_id', 'id', '`id`', '`id`', 20, 20, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // pasien_id
        $this->pasien_id = new DbField('kontak_darurat', 'kontak_darurat', 'x_pasien_id', 'pasien_id', '`pasien_id`', '`pasien_id`', 20, 20, -1, false, '`pasien_id`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        $this->pasien_id->IsForeignKey = true; // Foreign key field
        ...
        $this->pasien_id->Lookup = new Lookup('pasien_id', 'pasien', false, 'id', ["nama","jenis_kelamin","tanggal_lahir","no_hp"], [], [], [], [], [], [], '', '');
        ...
        // nama
        $this->nama = new DbField('kontak_darurat', 'kontak_darurat', 'x_nama', 'nama', '`nama`', '`nama`', 200, 255, -1, false, '`nama`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // no_hp
        $this->no_hp = new DbField('kontak_darurat', 'kontak_darurat', 'x_no_hp', 'no_hp', '`no_hp`', '`no_hp`', 200, 15, -1, false, '`no_hp`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function getMasterFilter()
    {
        ...
        if ($this->getCurrentMasterTable() == "pasien") {
            if ($this->pasien_id->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("`id`", $this->pasien_id->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilter()
    {
        ...
        if ($this->getCurrentMasterTable() == "pasien") {
            if ($this->pasien_id->getSessionValue() != "") {
                $detailFilter .= "" . GetForeignKeySql("`pasien_id`", $this->pasien_id->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    // Master filter
    public function sqlMasterFilter_pasien()
    {
        return "`id`=@id@";
    }
    // Detail filter
    public function sqlDetailFilter_pasien()
    {
        return "`pasien_id`=@pasien_id@";
    }

    // Table level SQL
    public function getSqlFrom() // From
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`kontak_darurat`";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("kontakdaruratlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "kontakdaruratview") {
            return $Language->phrase("View");
        } elseif ($pageName == "kontakdaruratedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "kontakdaruratadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "KontakDaruratAdd";
            case Config("API_EDIT_ACTION"):
                return "KontakDaruratEdit";
            case Config("API_DELETE_ACTION"):
                return "KontakDaruratDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("kontakdaruratview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("kontakdaruratview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "kontakdaruratadd?" . $this->getUrlParm($parm);
        } else {
            $url = "kontakdaruratadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("kontakdaruratedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("kontakdaruratadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("kontakdaruratdelete", $this->getUrlParm());
    }

    // Add master url
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "pasien" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->pasien_id->CurrentValue ?? $this->pasien_id->getSessionValue());
        }
        return $url;
    }
