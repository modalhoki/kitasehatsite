...
/**
 * Table class for fasilitas_rumah_sakit
 */
class FasilitasRumahSakit extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'fasilitas_rumah_sakit';
        $this->TableName = 'fasilitas_rumah_sakit';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`fasilitas_rumah_sakit`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // fasilitas_id
        $this->fasilitas_id = new DbField('fasilitas_rumah_sakit', 'fasilitas_rumah_sakit', 'x_fasilitas_id', 'fasilitas_id', '`fasilitas_id`', '`fasilitas_id`', 3, 11, -1, false, '`fasilitas_id`', false, false, false, 'FORMATTED TEXT', 'SELECT');
    ...
    public function getMasterFilter()
    {
        ...
        if ($this->getCurrentMasterTable() == "rumah_sakit") {
            if ($this->rumah_sakit_id->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("`id`", $this->rumah_sakit_id->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilter()
    {
        ...
        if ($this->getCurrentMasterTable() == "rumah_sakit") {
            if ($this->rumah_sakit_id->getSessionValue() != "") {
                $detailFilter .= "" . GetForeignKeySql("`rumah_sakit_id`", $this->rumah_sakit_id->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    // Master filter
    public function sqlMasterFilter_rumah_sakit()
    {
        return "`id`=@id@";
    }
    // Detail filter
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`fasilitas_rumah_sakit`";
    }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'praktik_poli'
        $dtlrows = Container("praktik_poli")->loadRs("`fasilitas_rumah_sakit_id` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("praktik_poli")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("praktik_poli")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("praktik_poli")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("fasilitasrumahsakitlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "fasilitasrumahsakitedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "fasilitasrumahsakitadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "FasilitasRumahSakitView";
            case Config("API_ADD_ACTION"):
                return "FasilitasRumahSakitAdd";
            case Config("API_EDIT_ACTION"):
                return "FasilitasRumahSakitEdit";
            case Config("API_DELETE_ACTION"):
                return "FasilitasRumahSakitDelete";
            case Config("API_LIST_ACTION"):
                return "FasilitasRumahSakitList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("fasilitasrumahsakitview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("fasilitasrumahsakitview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "fasilitasrumahsakitadd?" . $this->getUrlParm($parm);
        } else {
            $url = "fasilitasrumahsakitadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("fasilitasrumahsakitdelete", $this->getUrlParm());
    }

    // Add master url
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "rumah_sakit" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->rumah_sakit_id->CurrentValue ?? $this->rumah_sakit_id->getSessionValue());
        }
        return $url;
    }
