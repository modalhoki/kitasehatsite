...
class Dokter extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // nama_dokter
        $this->nama_dokter = new DbField('dokter', 'dokter', 'x_nama_dokter', 'nama_dokter', '`nama_dokter`', '`nama_dokter`', 200, 50, -1, false, '`nama_dokter`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->nama_dokter->Nullable = false; // NOT NULL field
        $this->nama_dokter->Required = true; // Required field
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`dokter`";
    }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        $dtlrows = Container("praktik_poli")->loadRs("`dokter_id` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("dokterlist");
    }
    ...
    {
        global $Language;
        if ($pageName == "dokterview") {
            return $Language->phrase("View");
        } elseif ($pageName == "dokteredit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "dokteradd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("dokterview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("dokterview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "dokteradd?" . $this->getUrlParm($parm);
        } else {
            $url = "dokteradd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        return $this->keyUrl("dokterdelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        if (!$this->nama_dokter->Raw) {
            $this->nama_dokter->CurrentValue = HtmlDecode($this->nama_dokter->CurrentValue);
        }
