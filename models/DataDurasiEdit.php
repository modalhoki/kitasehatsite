 ...
 * Page class
 */
class DataDurasiEdit extends DataDurasi
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DataDurasiEdit";
    ...
    protected function loadFormValues()
    {
        ...
        $val = $CurrentForm->hasValue("waktu_edit") ? $CurrentForm->getValue("waktu_edit") : $CurrentForm->getValue("x_waktu_edit");
        if (!$this->waktu_edit->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->waktu_edit->Visible = false; // Disable update for API request
            } else {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->waktu_edit->EditValue = HtmlEncode(FormatDateTime($this->waktu_edit->CurrentValue, 8));
    ...
    protected function validateForm()
    {
        ...
        if ($this->waktu_edit->Required) {
            if (!$this->waktu_edit->IsDetailKey && EmptyValue($this->waktu_edit->FormValue)) {
                $this->waktu_edit->addErrorMessage(str_replace("%s", $this->waktu_edit->caption(), $this->waktu_edit->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->waktu_edit->FormValue)) {
            $this->waktu_edit->addErrorMessage($this->waktu_edit->getErrorMessage(false));
        }
        ...
        if (!CheckInteger($this->durasi->FormValue)) {
            $this->durasi->addErrorMessage($this->durasi->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // jalur
            $this->jalur->setDbValueDef($rsnew, $this->jalur->CurrentValue, "", $this->jalur->ReadOnly);
