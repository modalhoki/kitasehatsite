 ...
 * Page class
 */
class RumahSakitView extends RumahSakit
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RumahSakitView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id") ?? Route("id")) !== null) {
                ...
                $returnUrl = "rumahsakitlist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "rumahsakitlist"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "rumahsakitlist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $item = &$option->add("detail_fasilitas_rumah_sakit");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("fasilitas_rumah_sakit", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("fasilitasrumahsakitlist?" . Config("TABLE_SHOW_MASTER") . "=rumah_sakit&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("FasilitasRumahSakitGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'rumah_sakit')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=fasilitas_rumah_sakit"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "fasilitas_rumah_sakit";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'rumah_sakit')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=fasilitas_rumah_sakit"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailEditLink")) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "fasilitas_rumah_sakit";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'rumah_sakit')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=fasilitas_rumah_sakit"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailCopyLink")) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
        ...
        $item->Body = $body;
        $item->Visible = $Security->allowList(CurrentProjectID() . 'fasilitas_rumah_sakit');
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("fasilitas_rumah_sakit", $detailTblVar)) {
                $detailPageObj = Container("FasilitasRumahSakitGrid");
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->rumah_sakit_id->IsDetailKey = true;
                    $detailPageObj->rumah_sakit_id->CurrentValue = $this->id->CurrentValue;
                    $detailPageObj->rumah_sakit_id->setSessionValue($detailPageObj->rumah_sakit_id->CurrentValue);
                }
            }
        }
    }
