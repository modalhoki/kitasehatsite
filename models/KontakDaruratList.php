 ...
 * Page class
 */
class KontakDaruratList extends KontakDarurat
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'kontak_darurat';

    // Page object name
    public $PageObjName = "KontakDaruratList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fkontak_daruratlist";
    ...
    public function __construct()
    {
        ...
        // Table object (kontak_darurat)
        if (!isset($GLOBALS["kontak_darurat"]) || get_class($GLOBALS["kontak_darurat"]) == PROJECT_NAMESPACE . "kontak_darurat") {
            $GLOBALS["kontak_darurat"] = &$this;
        }
        ...
        $this->AddUrl = "kontakdaruratadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "kontakdaruratdelete";
        $this->MultiUpdateUrl = "kontakdaruratupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'kontak_darurat');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fkontak_daruratlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("kontak_darurat"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "pasien") {
            $masterTbl = Container("pasien");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("pasienlist"); // Return to master page
                return;
            } else {
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_no_hp") && $CurrentForm->hasValue("o_no_hp") && $this->no_hp->CurrentValue != $this->no_hp->OldValue) {
            return false;
        }
        return true;
    }
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fkontak_daruratlistsrch", $filters);
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetall") {
                ...
                $this->DbMasterFilter = "";
                $this->DbDetailFilter = "";
                        $this->pasien_id->setSessionValue("");
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fkontak_daruratlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fkontak_daruratlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fkontak_daruratlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->pasien_id->getSessionValue() != "") {
                $this->pasien_id->CurrentValue = GetForeignKeyValue($this->pasien_id->getSessionValue());
                $this->pasien_id->OldValue = $this->pasien_id->CurrentValue;
                $curVal = trim(strval($this->pasien_id->CurrentValue));
                if ($curVal != "") {
                    $this->pasien_id->ViewValue = $this->pasien_id->lookupCacheOption($curVal);
                ...
                if ($curVal != "") {
                    ...
                    $this->pasien_id->ViewValue = $this->pasien_id->Lookup !== null && is_array($this->pasien_id->Lookup->Options) ? $curVal : null;
                }
                if ($this->pasien_id->ViewValue !== null) { // Load from cache
                    $this->pasien_id->EditValue = array_values($this->pasien_id->Lookup->Options);
                    if ($this->pasien_id->ViewValue == "") {
                        $this->pasien_id->ViewValue = $Language->phrase("PleaseSelect");
                    }
                } else { // Lookup from database
                    if ($curVal == "") {
                        $filterWrk = "0=1";
                    } else {
                        $filterWrk = "`id`" . SearchString("=", $this->pasien_id->CurrentValue, DATATYPE_NUMBER, "");
                    }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // pasien_id
            if ($this->pasien_id->getSessionValue() != "") {
                $this->pasien_id->ReadOnly = true;
            }
            $this->pasien_id->setDbValueDef($rsnew, $this->pasien_id->CurrentValue, 0, $this->pasien_id->ReadOnly);
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['pasien_id']); // pasien_id
        $hash .= GetFieldHash($row['nama']); // nama
        $hash .= GetFieldHash($row['no_hp']); // no_hp
        return md5($hash);
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // pasien_id
        $this->pasien_id->setDbValueDef($rsnew, $this->pasien_id->CurrentValue, 0, false);
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fkontak_daruratlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "pasien") {
                $validMaster = true;
                $masterTbl = Container("pasien");
                if (($parm = Get("fk_id", Get("pasien_id"))) !== null) {
                    $masterTbl->id->setQueryStringValue($parm);
                    $this->pasien_id->setQueryStringValue($masterTbl->id->QueryStringValue);
                    $this->pasien_id->setSessionValue($this->pasien_id->QueryStringValue);
            ...
            if ($masterTblVar == "pasien") {
                $validMaster = true;
                $masterTbl = Container("pasien");
                if (($parm = Post("fk_id", Post("pasien_id"))) !== null) {
                    $masterTbl->id->setFormValue($parm);
                    $this->pasien_id->setFormValue($masterTbl->id->FormValue);
                    $this->pasien_id->setSessionValue($this->pasien_id->FormValue);
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "pasien") {
                if ($this->pasien_id->CurrentValue == "") {
                    $this->pasien_id->setSessionValue("");
                }
            }
        }
