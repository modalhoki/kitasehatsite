...
class PasienAdd extends Pasien
{
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->foto_profil->CurrentValue = null;
        $this->foto_profil->OldValue = $this->foto_profil->CurrentValue;
        $this->foto_profil_par_id->CurrentValue = null;
        $this->foto_profil_par_id->OldValue = $this->foto_profil_par_id->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'status_perkawinan' first before field var 'x_status_perkawinan'
        $val = $CurrentForm->hasValue("status_perkawinan") ? $CurrentForm->getValue("status_perkawinan") : $CurrentForm->getValue("x_status_perkawinan");
        if (!$this->status_perkawinan->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->status_perkawinan->Visible = false; // Disable update for API request
            } else {
                $this->status_perkawinan->setFormValue($val);
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['foto_profil'] = $this->foto_profil->CurrentValue;
        $row['foto_profil_par_id'] = $this->foto_profil_par_id->CurrentValue;
        return $row;
    }
    ...
    protected function validateForm()
    {
        ...
        if ($this->pendidikan->Required) {
            if ($this->pendidikan->FormValue == "") {
                $this->pendidikan->addErrorMessage(str_replace("%s", $this->pendidikan->caption(), $this->pendidikan->RequiredErrorMessage));
            }
        }
        if ($this->status_perkawinan->Required) {
            if ($this->status_perkawinan->FormValue == "") {
                $this->status_perkawinan->addErrorMessage(str_replace("%s", $this->status_perkawinan->caption(), $this->status_perkawinan->RequiredErrorMessage));
            }
        }
        ...
        if (in_array("kontak_darurat", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // foto_profil
        $this->foto_profil->setDbValueDef($rsnew, $this->foto_profil->CurrentValue, null, false);

        // foto_profil_par_id
        $this->foto_profil_par_id->setDbValueDef($rsnew, $this->foto_profil_par_id->CurrentValue, null, false);
        ...
        if ($addRow) {
            ...
            $detailPage = Container("KontakDaruratGrid");
            if (in_array("kontak_darurat", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->pasien_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "kontak_darurat"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->pasien_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
