 ...
 * Page class
 */
class DokterPoliAdd extends DokterPoli
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'dokter_poli';

    // Page object name
    public $PageObjName = "DokterPoliAdd";
    ...
    public function __construct()
    {
        ...
        // Table object (dokter_poli)
        if (!isset($GLOBALS["dokter_poli"]) || get_class($GLOBALS["dokter_poli"]) == PROJECT_NAMESPACE . "dokter_poli") {
            $GLOBALS["dokter_poli"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'dokter_poli');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("dokter_poli"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->fasilitas_rs_id->setVisibility();
        ...
        $this->setupLookupOptions($this->fasilitas_rs_id);
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("dokterpolilist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "dokterpolilist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "dokterpoliview") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->fasilitas_rs_id->CurrentValue = null;
        $this->fasilitas_rs_id->OldValue = $this->fasilitas_rs_id->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'fasilitas_rs_id' first before field var 'x_fasilitas_rs_id'
        $val = $CurrentForm->hasValue("fasilitas_rs_id") ? $CurrentForm->getValue("fasilitas_rs_id") : $CurrentForm->getValue("x_fasilitas_rs_id");
        if (!$this->fasilitas_rs_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->fasilitas_rs_id->Visible = false; // Disable update for API request
            } else {
                $this->fasilitas_rs_id->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->fasilitas_rs_id->CurrentValue = $this->fasilitas_rs_id->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['fasilitas_rs_id'] = $this->fasilitas_rs_id->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->fasilitas_rs_id->ViewValue = $this->fasilitas_rs_id->Lookup !== null && is_array($this->fasilitas_rs_id->Lookup->Options) ? $curVal : null;
            }
            if ($this->fasilitas_rs_id->ViewValue !== null) { // Load from cache
                $this->fasilitas_rs_id->EditValue = array_values($this->fasilitas_rs_id->Lookup->Options);
                if ($this->fasilitas_rs_id->ViewValue == "") {
                    $this->fasilitas_rs_id->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->fasilitas_rs_id->CurrentValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->fasilitas_rs_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->fasilitas_rs_id->Lookup->renderViewRow($rswrk[0]);
                ...
                foreach ($arwrk as &$row)
                    $row = $this->fasilitas_rs_id->Lookup->renderViewRow($row);
                $this->fasilitas_rs_id->EditValue = $arwrk;
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->fasilitas_rs_id->Required) {
            if (!$this->fasilitas_rs_id->IsDetailKey && EmptyValue($this->fasilitas_rs_id->FormValue)) {
                $this->fasilitas_rs_id->addErrorMessage(str_replace("%s", $this->fasilitas_rs_id->caption(), $this->fasilitas_rs_id->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // fasilitas_rs_id
        $this->fasilitas_rs_id->setDbValueDef($rsnew, $this->fasilitas_rs_id->CurrentValue, 0, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("dokterpolilist"), "", $this->TableVar, true);
