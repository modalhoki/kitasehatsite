...
loadjs.ready("head", function () {
    ...
    fpraktik_polilist.lists.fasilitas_rumah_sakit_id = <?= $Page->fasilitas_rumah_sakit_id->toClientList($Page) ?>;
...
<?php } ?>
    ...
    <?php } ?>
    <?php if ($Page->fasilitas_rumah_sakit_id->Visible) { // fasilitas_rumah_sakit_id ?>
        <td data-name="fasilitas_rumah_sakit_id" <?= $Page->fasilitas_rumah_sakit_id->cellAttributes() ?>>
<?php if ($Page->RowType == ROWTYPE_ADD) { // Add record ?>
<?php if ($Page->fasilitas_rumah_sakit_id->getSessionValue() != "") { ?>
<span id="el<?= $Page->RowCount ?>_praktik_poli_fasilitas_rumah_sakit_id" class="form-group">
...
<input type="hidden" id="x<?= $Page->RowIndex ?>_fasilitas_rumah_sakit_id" name="x<?= $Page->RowIndex ?>_fasilitas_rumah_sakit_id" value="<?= HtmlEncode($Page->fasilitas_rumah_sakit_id->CurrentValue) ?>" data-hidden="1">
<?php } else { ?>
<span id="el<?= $Page->RowCount ?>_praktik_poli_fasilitas_rumah_sakit_id" class="form-group">
<div class="input-group ew-lookup-list">
    <div class="form-control ew-lookup-text" tabindex="-1" id="lu_x<?= $Page->RowIndex ?>_fasilitas_rumah_sakit_id"><?= EmptyValue(strval($Page->fasilitas_rumah_sakit_id->ViewValue)) ? $Language->phrase("PleaseSelect") : $Page->fasilitas_rumah_sakit_id->ViewValue ?></div>
    <div class="input-group-append">
        <button type="button" title="<?= HtmlEncode(str_replace("%s", RemoveHtml($Page->fasilitas_rumah_sakit_id->caption()), $Language->phrase("LookupLink", true))) ?>" class="ew-lookup-btn btn btn-default"<?= ($Page->fasilitas_rumah_sakit_id->ReadOnly || $Page->fasilitas_rumah_sakit_id->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x<?= $Page->RowIndex ?>_fasilitas_rumah_sakit_id',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
        <?php if (AllowAdd(CurrentProjectID() . "fasilitas_rumah_sakit") && !$Page->fasilitas_rumah_sakit_id->ReadOnly) { ?>
        <button type="button" class="btn btn-default ew-add-opt-btn" id="aol_x<?= $Page->RowIndex ?>_fasilitas_rumah_sakit_id" title="<?= HtmlTitle($Language->phrase("AddLink")) . "&nbsp;" . $Page->fasilitas_rumah_sakit_id->caption() ?>" data-title="<?= $Page->fasilitas_rumah_sakit_id->caption() ?>" onclick="ew.addOptionDialogShow({lnk:this,el:'x<?= $Page->RowIndex ?>_fasilitas_rumah_sakit_id',url:'<?= GetUrl("fasilitasrumahsakitaddopt") ?>'});"><i class="fas fa-plus ew-icon"></i></button>
        <?php } ?>
    </div>
</div>
<div class="invalid-feedback"><?= $Page->fasilitas_rumah_sakit_id->getErrorMessage() ?></div>
<?= $Page->fasilitas_rumah_sakit_id->Lookup->getParamTag($Page, "p_x" . $Page->RowIndex . "_fasilitas_rumah_sakit_id") ?>
<input type="hidden" is="selection-list" data-table="praktik_poli" data-field="x_fasilitas_rumah_sakit_id" data-type="text" data-multiple="0" data-lookup="1" data-value-separator="<?= $Page->fasilitas_rumah_sakit_id->displayValueSeparatorAttribute() ?>" name="x<?= $Page->RowIndex ?>_fasilitas_rumah_sakit_id" id="x<?= $Page->RowIndex ?>_fasilitas_rumah_sakit_id" value="<?= $Page->fasilitas_rumah_sakit_id->CurrentValue ?>"<?= $Page->fasilitas_rumah_sakit_id->editAttributes() ?>>
</span>
<?php } ?>
...
<?php } ?>
<?php if ($Page->RowType == ROWTYPE_VIEW) { // View record ?>
<span id="el<?= $Page->RowCount ?>_praktik_poli_fasilitas_rumah_sakit_id">
<span<?= $Page->fasilitas_rumah_sakit_id->viewAttributes() ?>>
...
<input type="<?= $Page->jam_praktik->getInputTextType() ?>" data-table="praktik_poli" data-field="x_jam_praktik" name="x<?= $Page->RowIndex ?>_jam_praktik" id="x<?= $Page->RowIndex ?>_jam_praktik" size="30" maxlength="13" placeholder="<?= HtmlEncode($Page->jam_praktik->getPlaceHolder()) ?>" value="<?= $Page->jam_praktik->EditValue ?>"<?= $Page->jam_praktik->editAttributes() ?>>
