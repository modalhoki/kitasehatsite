...
<input type="<?= $Page->waktu->getInputTextType() ?>" data-table="antrean_umum_rs" data-field="x_waktu" data-format="1" name="x_waktu" id="x_waktu" placeholder="<?= HtmlEncode($Page->waktu->getPlaceHolder()) ?>" value="<?= $Page->waktu->EditValue ?>"<?= $Page->waktu->editAttributes() ?>>
...
loadjs.ready(["fantrean_umum_rssearch", "datetimepicker"], function() {
    ew.createDateTimePicker("fantrean_umum_rssearch", "x_waktu", {"ignoreReadonly":true,"useCurrent":false,"format":1});
});
</script>
<?php } ?>
</span>
