...
var fwebusers_rsedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fwebusers_rsedit = currentForm = new ew.Form("fwebusers_rsedit", "edit");

    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "webusers_rs")) ?>,
        fields = currentTable.fields;
    if (!ew.vars.tables.webusers_rs)
        ew.vars.tables.webusers_rs = currentTable;
    fwebusers_rsedit.addFields([
    ...
    // Validate form
    fwebusers_rsedit.validate = function () {
    ...
    // Form_CustomValidate
    fwebusers_rsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fwebusers_rsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fwebusers_rsedit.lists.administrator_rumah_sakit = <?= $Page->administrator_rumah_sakit->toClientList($Page) ?>;
    loadjs.done("fwebusers_rsedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fwebusers_rsedit" id="fwebusers_rsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_webusers_rs_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_webusers_rs_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="webusers_rs" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
<?php if ($Page->_username->Visible) { // username ?>
    <div id="r__username" class="form-group row">
        <label id="elh_webusers_rs__username" for="x__username" class="<?= $Page->LeftColumnClass ?>"><?= $Page->_username->caption() ?><?= $Page->_username->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->_username->cellAttributes() ?>>
<span id="el_webusers_rs__username">
<input type="<?= $Page->_username->getInputTextType() ?>" data-table="webusers_rs" data-field="x__username" name="x__username" id="x__username" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->_username->getPlaceHolder()) ?>" value="<?= $Page->_username->EditValue ?>"<?= $Page->_username->editAttributes() ?> aria-describedby="x__username_help">
...
<?php if ($Page->_password->Visible) { // password ?>
    <div id="r__password" class="form-group row">
        <label id="elh_webusers_rs__password" for="x__password" class="<?= $Page->LeftColumnClass ?>"><?= $Page->_password->caption() ?><?= $Page->_password->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->_password->cellAttributes() ?>>
<span id="el_webusers_rs__password">
...
<?php if ($Page->role->Visible) { // role ?>
    <div id="r_role" class="form-group row">
        <label id="elh_webusers_rs_role" for="x_role" class="<?= $Page->LeftColumnClass ?>"><?= $Page->role->caption() ?><?= $Page->role->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->role->cellAttributes() ?>>
<span id="el_webusers_rs_role">
    <select
        id="x_role"
        name="x_role"
...
<?php if ($Page->rumah_sakit_id->Visible) { // rumah_sakit_id ?>
    <div id="r_rumah_sakit_id" class="form-group row">
        <label id="elh_webusers_rs_rumah_sakit_id" for="x_rumah_sakit_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->rumah_sakit_id->caption() ?><?= $Page->rumah_sakit_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->rumah_sakit_id->cellAttributes() ?>>
<span id="el_webusers_rs_rumah_sakit_id">
...
<input type="hidden" data-table="webusers_rs" data-field="x_rumah_sakit_id" data-hidden="1" name="x_rumah_sakit_id" id="x_rumah_sakit_id" value="<?= HtmlEncode($Page->rumah_sakit_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
<?php if ($Page->administrator_rumah_sakit->Visible) { // administrator_rumah_sakit ?>
    <div id="r_administrator_rumah_sakit" class="form-group row">
        <label id="elh_webusers_rs_administrator_rumah_sakit" for="x_administrator_rumah_sakit" class="<?= $Page->LeftColumnClass ?>"><?= $Page->administrator_rumah_sakit->caption() ?><?= $Page->administrator_rumah_sakit->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->administrator_rumah_sakit->cellAttributes() ?>>
<span id="el_webusers_rs_administrator_rumah_sakit">
<div class="input-group ew-lookup-list" aria-describedby="x_administrator_rumah_sakit_help">
    <div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_administrator_rumah_sakit"><?= EmptyValue(strval($Page->administrator_rumah_sakit->ViewValue)) ? $Language->phrase("PleaseSelect") : $Page->administrator_rumah_sakit->ViewValue ?></div>
