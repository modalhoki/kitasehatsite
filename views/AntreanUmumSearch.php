...
// Page object
$AntreanUmumSearch = &$Page;
?>
<script>
var currentForm, currentPageID;
var fantrean_umumsearch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object for search
    <?php if ($Page->IsModal) { ?>
    fantrean_umumsearch = currentAdvancedSearchForm = new ew.Form("fantrean_umumsearch", "search");
    <?php } else { ?>
    fantrean_umumsearch = currentForm = new ew.Form("fantrean_umumsearch", "search");
    <?php } ?>
    currentPageID = ew.PAGE_ID = "search";

    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "antrean_umum")) ?>,
        fields = currentTable.fields;
    fantrean_umumsearch.addFields([
    ...
    // Set invalid fields
    $(function() {
        fantrean_umumsearch.setInvalid();
    });

    // Validate form
    fantrean_umumsearch.validate = function () {
    ...
    // Form_CustomValidate
    fantrean_umumsearch.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fantrean_umumsearch.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fantrean_umumsearch.lists.pasien_id = <?= $Page->pasien_id->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fantrean_umumsearch" id="fantrean_umumsearch" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label for="x_id" class="<?= $Page->LeftColumnClass ?>"><span id="elh_antrean_umum_id"><?= $Page->id->caption() ?></span>
...
<input type="hidden" name="z_id" id="z_id" value="=">
        ...
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
            <span id="el_antrean_umum_id" class="ew-search-field ew-search-field-single">
<input type="<?= $Page->id->getInputTextType() ?>" data-table="antrean_umum" data-field="x_id" name="x_id" id="x_id" placeholder="<?= HtmlEncode($Page->id->getPlaceHolder()) ?>" value="<?= $Page->id->EditValue ?>"<?= $Page->id->editAttributes() ?>>
...
<?php if ($Page->nomor_antrean->Visible) { // nomor_antrean ?>
    <div id="r_nomor_antrean" class="form-group row">
        <label for="x_nomor_antrean" class="<?= $Page->LeftColumnClass ?>"><span id="elh_antrean_umum_nomor_antrean"><?= $Page->nomor_antrean->caption() ?></span>
        <span class="ew-search-operator">
<?= $Language->phrase("=") ?>
<input type="hidden" name="z_nomor_antrean" id="z_nomor_antrean" value="=">
        ...
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->nomor_antrean->cellAttributes() ?>>
            <span id="el_antrean_umum_nomor_antrean" class="ew-search-field ew-search-field-single">
<input type="<?= $Page->nomor_antrean->getInputTextType() ?>" data-table="antrean_umum" data-field="x_nomor_antrean" name="x_nomor_antrean" id="x_nomor_antrean" size="30" placeholder="<?= HtmlEncode($Page->nomor_antrean->getPlaceHolder()) ?>" value="<?= $Page->nomor_antrean->EditValue ?>"<?= $Page->nomor_antrean->editAttributes() ?>>
...
<?php if ($Page->waktu->Visible) { // waktu ?>
    <div id="r_waktu" class="form-group row">
        <label for="x_waktu" class="<?= $Page->LeftColumnClass ?>"><span id="elh_antrean_umum_waktu"><?= $Page->waktu->caption() ?></span>
        <span class="ew-search-operator">
<?= $Language->phrase("=") ?>
<input type="hidden" name="z_waktu" id="z_waktu" value="=">
        ...
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->waktu->cellAttributes() ?>>
            <span id="el_antrean_umum_waktu" class="ew-search-field ew-search-field-single">
<input type="<?= $Page->waktu->getInputTextType() ?>" data-table="antrean_umum" data-field="x_waktu" data-format="1" name="x_waktu" id="x_waktu" placeholder="<?= HtmlEncode($Page->waktu->getPlaceHolder()) ?>" value="<?= $Page->waktu->EditValue ?>"<?= $Page->waktu->editAttributes() ?>>
...
<script>
loadjs.ready(["fantrean_umumsearch", "datetimepicker"], function() {
    ew.createDateTimePicker("fantrean_umumsearch", "x_waktu", {"ignoreReadonly":true,"useCurrent":false,"format":1});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->pasien_id->Visible) { // pasien_id ?>
    <div id="r_pasien_id" class="form-group row">
        <label for="x_pasien_id" class="<?= $Page->LeftColumnClass ?>"><span id="elh_antrean_umum_pasien_id"><?= $Page->pasien_id->caption() ?></span>
        <span class="ew-search-operator">
<?= $Language->phrase("=") ?>
<input type="hidden" name="z_pasien_id" id="z_pasien_id" value="=">
        ...
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->pasien_id->cellAttributes() ?>>
            <span id="el_antrean_umum_pasien_id" class="ew-search-field ew-search-field-single">
...
<?php if ($Page->fasilitas_id->Visible) { // fasilitas_id ?>
    <div id="r_fasilitas_id" class="form-group row">
        <label for="x_fasilitas_id" class="<?= $Page->LeftColumnClass ?>"><span id="elh_antrean_umum_fasilitas_id"><?= $Page->fasilitas_id->caption() ?></span>
        <span class="ew-search-operator">
<?= $Language->phrase("=") ?>
<input type="hidden" name="z_fasilitas_id" id="z_fasilitas_id" value="=">
        ...
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->fasilitas_id->cellAttributes() ?>>
            <span id="el_antrean_umum_fasilitas_id" class="ew-search-field ew-search-field-single">
<div class="input-group ew-lookup-list">
...
<?php if ($Page->rumah_sakit_id->Visible) { // rumah_sakit_id ?>
    <div id="r_rumah_sakit_id" class="form-group row">
        <label class="<?= $Page->LeftColumnClass ?>"><span id="elh_antrean_umum_rumah_sakit_id"><?= $Page->rumah_sakit_id->caption() ?></span>
        <span class="ew-search-operator">
<?= $Language->phrase("=") ?>
<input type="hidden" name="z_rumah_sakit_id" id="z_rumah_sakit_id" value="=">
        ...
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->rumah_sakit_id->cellAttributes() ?>>
            <span id="el_antrean_umum_rumah_sakit_id" class="ew-search-field ew-search-field-single">
<?php
...
<?php if ($Page->status->Visible) { // status ?>
    <div id="r_status" class="form-group row">
        <label class="<?= $Page->LeftColumnClass ?>"><span id="elh_antrean_umum_status"><?= $Page->status->caption() ?></span>
        <span class="ew-search-operator">
<?= $Language->phrase("=") ?>
<input type="hidden" name="z_status" id="z_status" value="=">
        ...
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->status->cellAttributes() ?>>
            <span id="el_antrean_umum_status" class="ew-search-field ew-search-field-single">
...
<?php if ($Page->keluhan_awal->Visible) { // keluhan_awal ?>
    <div id="r_keluhan_awal" class="form-group row">
        <label for="x_keluhan_awal" class="<?= $Page->LeftColumnClass ?>"><span id="elh_antrean_umum_keluhan_awal"><?= $Page->keluhan_awal->caption() ?></span>
        <span class="ew-search-operator">
...
<input type="hidden" name="z_webusers_id" id="z_webusers_id" value="=">
    ...
    <select
        ...
        data-select2-id="antrean_umum_x_webusers_id"
        data-table="antrean_umum"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='antrean_umum_x_webusers_id']"),
        options = { name: "x_webusers_id", selectId: "antrean_umum_x_webusers_id", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.antrean_umum.fields.webusers_id.selectOptions);
...
loadjs.ready("head", function() {
    ew.addEventHandlers("antrean_umum");
});
</script>
<script>
