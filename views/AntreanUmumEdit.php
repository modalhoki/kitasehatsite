...
// Page object
$AntreanUmumEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fantrean_umumedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fantrean_umumedit = currentForm = new ew.Form("fantrean_umumedit", "edit");

    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "antrean_umum")) ?>,
        fields = currentTable.fields;
    if (!ew.vars.tables.antrean_umum)
        ew.vars.tables.antrean_umum = currentTable;
    fantrean_umumedit.addFields([
    ...
    // Validate form
    fantrean_umumedit.validate = function () {
    ...
    // Form_CustomValidate
    fantrean_umumedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fantrean_umumedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fantrean_umumedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    fantrean_umumedit.lists.webusers_id = <?= $Page->webusers_id->toClientList($Page) ?>;
    loadjs.done("fantrean_umumedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fantrean_umumedit" id="fantrean_umumedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->nomor_antrean->Visible) { // nomor_antrean ?>
    <div id="r_nomor_antrean" class="form-group row">
        <label id="elh_antrean_umum_nomor_antrean" for="x_nomor_antrean" class="<?= $Page->LeftColumnClass ?>"><?= $Page->nomor_antrean->caption() ?><?= $Page->nomor_antrean->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="antrean_umum" data-field="x_nomor_antrean" data-hidden="1" name="x_nomor_antrean" id="x_nomor_antrean" value="<?= HtmlEncode($Page->nomor_antrean->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
<?php if ($Page->waktu->Visible) { // waktu ?>
    <div id="r_waktu" class="form-group row">
        <label id="elh_antrean_umum_waktu" for="x_waktu" class="<?= $Page->LeftColumnClass ?>"><?= $Page->waktu->caption() ?><?= $Page->waktu->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="antrean_umum" data-field="x_waktu" data-hidden="1" name="x_waktu" id="x_waktu" value="<?= HtmlEncode($Page->waktu->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
<?php if ($Page->pasien_id->Visible) { // pasien_id ?>
    <div id="r_pasien_id" class="form-group row">
        <label id="elh_antrean_umum_pasien_id" for="x_pasien_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->pasien_id->caption() ?><?= $Page->pasien_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="antrean_umum" data-field="x_pasien_id" data-hidden="1" name="x_pasien_id" id="x_pasien_id" value="<?= HtmlEncode($Page->pasien_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
<?php if ($Page->fasilitas_id->Visible) { // fasilitas_id ?>
    <div id="r_fasilitas_id" class="form-group row">
        <label id="elh_antrean_umum_fasilitas_id" for="x_fasilitas_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->fasilitas_id->caption() ?><?= $Page->fasilitas_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="antrean_umum" data-field="x_fasilitas_id" data-hidden="1" name="x_fasilitas_id" id="x_fasilitas_id" value="<?= HtmlEncode($Page->fasilitas_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
<?php if ($Page->rumah_sakit_id->Visible) { // rumah_sakit_id ?>
    <div id="r_rumah_sakit_id" class="form-group row">
        <label id="elh_antrean_umum_rumah_sakit_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->rumah_sakit_id->caption() ?><?= $Page->rumah_sakit_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="antrean_umum" data-field="x_rumah_sakit_id" data-hidden="1" name="x_rumah_sakit_id" id="x_rumah_sakit_id" value="<?= HtmlEncode($Page->rumah_sakit_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
<?php if ($Page->status->Visible) { // status ?>
    <div id="r_status" class="form-group row">
        <label id="elh_antrean_umum_status" class="<?= $Page->LeftColumnClass ?>"><?= $Page->status->caption() ?><?= $Page->status->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php if ($Page->webusers_id->Visible) { // webusers_id ?>
    <div id="r_webusers_id" class="form-group row">
        <label id="elh_antrean_umum_webusers_id" for="x_webusers_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->webusers_id->caption() ?><?= $Page->webusers_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php } ?>
    ...
    <input type="hidden" data-table="antrean_umum" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
