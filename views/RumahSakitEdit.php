...
var frumah_sakitedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    frumah_sakitedit = currentForm = new ew.Form("frumah_sakitedit", "edit");
    ...
    // Validate form
    frumah_sakitedit.validate = function () {
    ...
    // Form_CustomValidate
    frumah_sakitedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    frumah_sakitedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    frumah_sakitedit.lists.daerah_id = <?= $Page->daerah_id->toClientList($Page) ?>;
    loadjs.done("frumah_sakitedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="frumah_sakitedit" id="frumah_sakitedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="<?= $Page->jam_buka->getInputTextType() ?>" data-table="rumah_sakit" data-field="x_jam_buka" name="x_jam_buka" id="x_jam_buka" size="30" maxlength="13" placeholder="<?= HtmlEncode($Page->jam_buka->getPlaceHolder()) ?>" value="<?= $Page->jam_buka->EditValue ?>"<?= $Page->jam_buka->editAttributes() ?> aria-describedby="x_jam_buka_help">
