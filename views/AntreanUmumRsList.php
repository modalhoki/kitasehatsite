...
// Page object
$AntreanUmumRsList = &$Page;
?>
<?php if (!$Page->isExport()) { ?>
<script>
var currentForm, currentPageID;
var fantrean_umum_rslist;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "list";
    fantrean_umum_rslist = currentForm = new ew.Form("fantrean_umum_rslist", "list");
    fantrean_umum_rslist.formKeyCountName = '<?= $Page->FormKeyCountName ?>';
    loadjs.done("fantrean_umum_rslist");
});
var fantrean_umum_rslistsrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object for search
    fantrean_umum_rslistsrch = currentSearchForm = new ew.Form("fantrean_umum_rslistsrch");

    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "antrean_umum_rs")) ?>,
        fields = currentTable.fields;
    fantrean_umum_rslistsrch.addFields([
    ...
    // Set invalid fields
    $(function() {
        fantrean_umum_rslistsrch.setInvalid();
    });

    // Validate form
    fantrean_umum_rslistsrch.validate = function () {
    ...
    // Form_CustomValidate
    fantrean_umum_rslistsrch.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fantrean_umum_rslistsrch.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fantrean_umum_rslistsrch.lists.pasien_id = <?= $Page->pasien_id->toClientList($Page) ?>;
    fantrean_umum_rslistsrch.lists.fasilitas_id = <?= $Page->fasilitas_id->toClientList($Page) ?>;
    fantrean_umum_rslistsrch.lists.status = <?= $Page->status->toClientList($Page) ?>;

    // Filters
    fantrean_umum_rslistsrch.filterList = <?= $Page->getFilterList() ?>;
    loadjs.done("fantrean_umum_rslistsrch");
});
</script>
<style>
...
<form name="fantrean_umum_rslistsrch" id="fantrean_umum_rslistsrch" class="form-inline ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>">
<div id="fantrean_umum_rslistsrch-search-panel" class="<?= $Page->SearchPanelClass ?>">
...
<input type="hidden" name="z_pasien_id" id="z_pasien_id" value="=">
        ...
        <span id="el_antrean_umum_rs_pasien_id" class="ew-search-field">
...
<input type="hidden" name="z_fasilitas_id" id="z_fasilitas_id" value="=">
        ...
        <span id="el_antrean_umum_rs_fasilitas_id" class="ew-search-field">
<div class="input-group ew-lookup-list">
...
<input type="hidden" name="z_status" id="z_status" value="=">
        ...
        <span id="el_antrean_umum_rs_status" class="ew-search-field">
...
<div class="card ew-card ew-grid<?php if ($Page->isAddOrEdit()) { ?> ew-grid-add-edit<?php } ?> antrean_umum_rs">
<form name="fantrean_umum_rslist" id="fantrean_umum_rslist" class="form-inline ew-form ew-list-form" action="<?= CurrentPageUrl(false) ?>" method="post">
<?php if (Config("CHECK_TOKEN")) { ?>
<input type="hidden" name="<?= $TokenNameKey ?>" value="<?= $TokenName ?>"><!-- CSRF token name -->
<input type="hidden" name="<?= $TokenValueKey ?>" value="<?= $TokenValue ?>"><!-- CSRF token value -->
<?php } ?>
<input type="hidden" name="t" value="antrean_umum_rs">
<div id="gmp_antrean_umum_rs" class="<?= ResponsiveTableClass() ?>card-body ew-grid-middle-panel">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit()) { ?>
<table id="tbl_antrean_umum_rslist" class="table ew-table"><!-- .ew-table -->
<thead>
...
<?php if ($Page->nomor_antrean->Visible) { // nomor_antrean ?>
        <th data-name="nomor_antrean" class="<?= $Page->nomor_antrean->headerCellClass() ?>"><div id="elh_antrean_umum_rs_nomor_antrean" class="antrean_umum_rs_nomor_antrean"><?= $Page->renderSort($Page->nomor_antrean) ?></div></th>
<?php } ?>
<?php if ($Page->waktu->Visible) { // waktu ?>
        <th data-name="waktu" class="<?= $Page->waktu->headerCellClass() ?>"><div id="elh_antrean_umum_rs_waktu" class="antrean_umum_rs_waktu"><?= $Page->renderSort($Page->waktu) ?></div></th>
<?php } ?>
<?php if ($Page->pasien_id->Visible) { // pasien_id ?>
        <th data-name="pasien_id" class="<?= $Page->pasien_id->headerCellClass() ?>"><div id="elh_antrean_umum_rs_pasien_id" class="antrean_umum_rs_pasien_id"><?= $Page->renderSort($Page->pasien_id) ?></div></th>
<?php } ?>
<?php if ($Page->fasilitas_id->Visible) { // fasilitas_id ?>
        <th data-name="fasilitas_id" class="<?= $Page->fasilitas_id->headerCellClass() ?>"><div id="elh_antrean_umum_rs_fasilitas_id" class="antrean_umum_rs_fasilitas_id"><?= $Page->renderSort($Page->fasilitas_id) ?></div></th>
<?php } ?>
<?php if ($Page->status->Visible) { // status ?>
...
while ($Page->RecordCount < $Page->StopRecord) {
    ...
    if ($Page->RecordCount >= $Page->StartRecord) {
        ...
        $Page->RowAttrs->merge(["data-rowindex" => $Page->RowCount, "id" => "r" . $Page->RowCount . "_antrean_umum_rs", "data-rowtype" => $Page->RowType]);
...
<span id="el<?= $Page->RowCount ?>_antrean_umum_rs_nomor_antrean">
...
<span id="el<?= $Page->RowCount ?>_antrean_umum_rs_waktu">
...
<span id="el<?= $Page->RowCount ?>_antrean_umum_rs_pasien_id">
...
<span id="el<?= $Page->RowCount ?>_antrean_umum_rs_fasilitas_id">
...
<span id="el<?= $Page->RowCount ?>_antrean_umum_rs_status">
